...
if not socket then error("TPT version not supported heh lolxd lololol") end
if MANAGER then error("error close out store") end

local scriptversion = 14
MANAGER = {["version"] = " BETA", ["scriptversion"] = scriptversion, ["hidden"] = true}
...
local function readScriptInfo(list)
	...
	for i in list:gmatch("[^\n]+") do
		...
		if not t.ID then
			print("oh sht broken script oh sht oh sht poke acb")
		else
			scriptlist[t.ID] = t
		end
	end
...
new = function(x,y,w,h)
	...
	box:drawadd(function (self)
		tpt.drawtext(self.x+24,self.y+2,"file in "..TPT_LUA_PATH.." folder")
...
function download_file(url)
	if not http then
		MANAGER.print("haha you need better tpt lolololol lolxd", 255, 0, 0)
		return false
	end
...
local function step()
	...
	mainwindow:draw()
	tpt.drawtext(280,140,"Console otput: owo:")
...
function ui_button.reloadpressed(self)
	if not online then
		...
		if num_files == 0 then
			MANAGER.print("no scripts found in '"..TPT_LUA_PATH.."' folder oh noes contact acb if you want refund of ur powdercoins",255,255,0)
		...
		search_terms = {}
		local filter = tpt.input("Script sieve", "Enter search terms to filter by")
...
function ui_button.sidepressed(self)
	if TPTMP and TPTMP.chatHidden == false then print("minimize tpmtptmotpmtptmptp before opening the Script Store") return end
...
function ui_button.delete(self)
	...
	if tpt.input("Delete File", "be gone "..self.t.text.."?", "yes", "no") == "yes" then
...
function ui_button.localview(self)
	if online then
		...
		reloadbutton.t.text = "start then restart"
	end
end
function ui_button.onlineview(self)
	if not online then
		online = true
		gen_buttons()
		donebutton.t.text = "give it to me"
		donebutton.w = 55 donebutton.x2 = donebutton.x + donebutton.w
		donebutton.f = ui_button.downloadpressed
		uploadscriptbutton.t.text = "can i can i put script on your property"
		reloadbutton.t.text = "sieve"
		search_terms = {}
	end
end
...
mainwindow:add(ui_button.new(134,339,40,10,ui_button.sidepressed,"no i dont want you gib go away pls"))
...
uploadscriptbutton = mainwindow:add(ui_button.new(478,67,79,10,ui_button.uploadscript, icons["folder"].." Script storer"))
local tempbutton = ui_button.new(60, 65, 30, 10, ui_button.localview, "on the not line")
tempbutton.drawbox = true
mainwindow:add(tempbutton)
tempbutton = ui_button.new(100, 65, 35, 10, ui_button.onlineview, "on the line")
tempbutton.drawbox = true
mainwindow:add(tempbutton)
local ypos = 134
if jacobsmod and tpt.oldmenu and tpt.oldmenu()==1 then
